...
export function QuestionList({ questions: initialQuestions }: QuestionListProps) {
  ...
  const handleToggleActive = async (q: Question & { answers: Answer[] }) => {
    const res = await fetch(`/api/questions/${q.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...q, is_active: !q.is_active, answers: q.answers }),
    })
    if (!res.ok) return
    const { question } = await res.json()
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-3 text-sm font-body">
        <span className="text-white/40">{questions.length} preguntas totales</span>
        ...
        <AnimatePresence>
          {filtered.map((question) => (
            <motion.div
              ...
              exit={{ opacity: 0, y: -4 }}
              className={cn(
                'rounded-lg border bg-bg-card overflow-hidden transition-colors duration-200',
                question.is_active ? 'border-border' : 'border-border/30 opacity-60'
              )}
            >
              {/* Question row */}
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <Button
                    size="icon"
                    variant={deletingId === question.id ? 'destructive' : 'ghost'}
                    className="h-7 w-7"
                    onClick={() => handleDelete(question.id)}
                    title={deletingId === question.id ? '¿Confirmar?' : 'Eliminar'}
                  >
              ...
              <AnimatePresence>
                {expandedId === question.id && editingId !== question.id && (
                  <motion.div
                    ...
                    className="border-t border-border/30 px-4 py-3 bg-bg-surface/50"
                  >
                    <div className="flex flex-col gap-1.5">
                      {question.answers
                        ...
                        .map((answer) => (
                          <div key={answer.id} className="flex items-center gap-3 text-sm">
                            <span className="w-5 h-5 flex items-center justify-center rounded bg-bg-elevated text-white/40 font-display text-xs shrink-0">
                              {answer.position}
                            </span>
                      ...
                      <div className="flex justify-between mt-1 pt-1 border-t border-border/20">
                        ...
                        <span className="font-display text-xs text-white/50">
                          {question.answers.reduce((s, a) => s + a.points, 0)} pts
                        </span>
                      </div>
                    </div>
                  </motion.div>
                ...
                {editingId === question.id && (
                  <motion.div
                    ...
                    animate={{ height: 'auto', opacity: 1 }}
                    exit={{ height: 0, opacity: 0 }}
                    className="border-t border-gold/30 bg-gold/5 px-4 py-4"
                  >
                    <QuestionForm
                      question={question}
